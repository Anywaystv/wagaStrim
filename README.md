...
While media is active, the stats row also shows the current **relay delay**.
With dynamic delay enabled, it shows the active catch-up rate in **ms/s** or
whether a jump is waiting for a keyframe. The rate is zero while the relay is
holding or growing the delay, even if a higher catch-up limit is saved.
These values describe relay scheduling; each viewer's browser adds its own
buffering and controls its playback speed. The control API exposes the same
values under `playout`.
